...
It may be worth reviewing logical expressions in Section \@ref(logicalexp) as they are used a lot in control flows. Pay attention to the `&` and `|` operators. In these examples, the operators were performed element-wise, and are useful when subsetting or modifying a data structure.  
...
if (a > 0 || a == 0){
  print("a is positive or 0")
}
```

The parenthesis after the `if` statement is the condition that has to be met for the code to be executed. In this example, the condition is met since `a` is equal to 1, so `a > 0` returns `TRUE`, and the code in the braces is executed. However, if we run the following, notice nothing gets printed.
...
The `ifelse()` function is a vectorized version of an if-else statement. It applies an if-else statement to each element in a vector, and returns a vector with the same length. For example, I have a number of students in class, and they pass if they score at least 60 on a test, otherwise they fail.
...
## Vectorization in R {#vectorization}
...
Consider the example in Section \@ref(forloops). We have a vector `1:10` and we want to square each value. Instead of squaring each entry in the vector one by one in a for loop, we can easily type `(1:10)^2` to get the same result.

As your code gets longer and more complex, making it more efficient with vectorization and pre-allocation will make a difference in how long your code runs. Another way of making code more efficient in R is using the `apply` family of functions: you have seen a simple example in Section \@ref(funcDF), and you will learn more about this family of functions in Section \@ref(applyfam).
